...
bool c_lw_http::post(std::wstring sURL, std::string& sReply, c_lw_httpd& PostData,std::vector<std::wstring> &headers)
{
	std::vector< BYTE > btReply;

	bool bResult = send_request(sURL, btReply, L"POST",
		(LPVOID)(PostData.get_data()), PostData.get_size(),headers);
