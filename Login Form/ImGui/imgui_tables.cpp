...
void ImGui::TableDrawContextMenu(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_Resizable)
    {
        if (column != NULL)
        {
            ...
            const bool canUnselectAll = (table->ColumnsEnabledCount != 1);
            if (MenuItem("Unselect All###Unselected", NULL, false, canUnselectAll))
            {
                for (int other_column_n = 1; other_column_n < table->ColumnsCount; other_column_n++)
                {
                    ImGuiTableColumn* other_column = &table->Columns[other_column_n];
                    other_column->IsUserEnabled = false;
                    other_column->IsUserEnabledNextFrame = false;
                }
            }
            const bool canSelectAll = (table->ColumnsEnabledCount < table->ColumnsCount);
            if (MenuItem("Select All###Select", NULL, false, canSelectAll))
            {
                for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
                {
                    ImGuiTableColumn* other_column = &table->Columns[other_column_n];
                    other_column->IsUserEnabled = true;
                    other_column->IsUserEnabledNextFrame = true;
                }
            }
        }
